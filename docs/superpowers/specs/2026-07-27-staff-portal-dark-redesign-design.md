...
Reskin the entire `/staff` shell to dark. The Overview page gets a bespoke layout update to match the approved mockup. The other five top-level pages (Conversations, Clients, Onboarding, Knowledge, Manage Staff) and their detail sub-pages (`clients/[id]`, `onboarding/[id]`) keep their current structure and markup — only the shared classes they already render pick up dark styling because those pages sit inside the newly-scoped wrapper.

That shared-class set is larger than just the obvious panel/button primitives, and spans three source files, not just `globals.css`:

- `app/globals.css`: `.panel`, `.panel-header`/`.panel-header h2` (rendered by the shared `PanelHeader` component inside nearly every `Panel` on all six top-level staff pages plus both detail sub-pages — hardcoded `color:#0c1732`/`#13233b`), `.stat-card` (and its `::before` gradient accent bar — see Implementation approach), `.eyebrow-row`, `.stat-value` (both used directly on the Overview stat tiles), `.primary-button`, `.secondary-button`, `.text-button`, `.modal-form`, `.knowledge-form`, `.detail-grid`, `.form-error`, `.support-composer`, `.support-bubble` (and its `.user`/`p` variants — the Conversations reply-thread message bubbles, hardcoded `background:#fff`/`color:#29405f`), `.empty-state`, `.beneficiary-list`, `.modal-copy`, `.knowledge-list`, `.agent-queue`, `.agent-intake-details`, `.new-conversation-panel`, `.client-search-results`
- `app/sections.css`: `.view-heading` (rendered by the shared `ViewHeading` component on every one of the six top-level staff pages via `<div className="section-view"><ViewHeading .../>` — its `h2`/`p` colors are dark navy, meant for a light page, so this is a page-title-on-every-page fix, not a corner case)
- `app/onboarding.css`: the full component family rendered by `/staff/onboarding/[id]` — `.onboarding-layout`, `.onboarding-checklist`, `.onboarding-form-card`, `.onboarding-form-head`, `.intake-fields`, `.onboarding-actions`, `.step-label`, `.overall-progress`. These use hardcoded light colors (e.g. `background:#fff`, `border:1px solid #d9e2ee`), not CSS variables, so today's design-system reskinning doesn't reach them either — they need the same `.staff-shell`-scoped override treatment as everything else.

The isolation approach (new block in `app/globals.css`, every selector scoped under `.staff-shell`) works the same regardless of which file defines the base rule being overridden — but an implementer should know to check all three files above for "what does this look like today," not just `globals.css`.
...
**Isolation.** Add a `.staff-shell` class alongside the existing `.app-shell` class on the root `<div>` in `app/staff/(shell)/layout.tsx` — additive, not a replacement, so the shared structural CSS (`.app-shell`'s flex/sizing rules etc.) still applies. All new dark styling is added as a new block in `app/globals.css`, with every selector scoped under `.staff-shell` (e.g. `.staff-shell .panel`, `.staff-shell .sidebar`, `.staff-shell .stat-card`, `.staff-shell .primary-button`). No edits to the existing unscoped `.app-shell` / `.sidebar` / `.panel` / `:root` rules or their design-system-application-layer overrides — those stay exactly as the client-portal redesign work leaves them. Every one of those existing rules is single-class or ancestor-scoped to a class that doesn't exist in the staff shell (e.g. `.call-script .panel`), so a two-class selector like `.staff-shell .panel` reliably wins on specificity regardless of source order — confirmed by checking every `.panel`/`.stat-card`/`.sidebar` rule across `globals.css`.

One unscoped rule needs an explicit override rather than just out-specificity-ing: `.stat-card::before` (globals.css:622) paints a blue→orange gradient bar on top of every `.stat-card` anywhere in the app. Left alone, it'll still render on the Overview page's three non-accent tiles, contradicting the flat-dark aesthetic. The new CSS block must include `.staff-shell .stat-card::before { content: none; }` (or a redraw matching the dark accent scheme) alongside the other overrides.

No staff page currently renders anything via `createPortal` or a modal-backdrop overlay — all forms (grant staff access, onboarding intake, etc.) render inline inside a `Panel` within the normal DOM tree under the root `.staff-shell` div. So the single scoping point in `layout.tsx` is sufficient today; revisit if a portal-rendered modal is ever added to a staff page.
