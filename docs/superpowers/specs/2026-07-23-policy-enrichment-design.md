...
- New `notifications` table: `id uuid, user_id uuid, type text, title text, message text, read boolean default false, related_id uuid null, created_at timestamptz`. `related_id` is a generic, unconstrained pointer (no FK — the row it points to depends on `type`, e.g. a policy id for `packet_delivered`/`premium_due_soon`) rather than a policy-specific column, so other features (e.g. a future agent-reply notification linking to a service request) can reuse this same table without a schema change. RLS: owner-only select/update (mark read), same pattern as every other client-owned table. Inserts happen via the admin client from trigger points (agent actions), same as `agent_notifications` today.
