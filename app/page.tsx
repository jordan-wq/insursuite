...
function AgentConsole() {
  ...
  type KnowledgeItem = { id: number; question: string };
  ...
  const update = async (id: number, status: string) => { await fetch("/api/agent/queue", { method: "PATCH", headers: { "content-type": "application/json" }, body: JSON.stringify({ id, status }) }); load(); };
...
function SectionContent({ active, onNavigate, onPolicy, onOpen, notify, policyData, uploadedDocuments, profile, isSample, requests, onCreateRequest, onSaveProfile }: { active: NavKey; onNavigate: (key: NavKey) => void; onPolicy: (policy: Policy) => void; onOpen: (modal: string) => void; notify: (message: string) => void; policyData: Policy[]; uploadedDocuments: PortalDocument[]; profile: StoredProfile | null; isSample: boolean; requests: ServiceRequest[]; onCreateRequest: (input: SupportRequestInput) => Promise<ServiceRequest | null>; onSaveProfile: (patch: Record<string, string | boolean>) => Promise<StoredProfile | null> }) {
  ...
  return <SettingsView notify={notify} />;
}
...
const intakeSections: { title: string; short: string; description: string; fields: IntakeField[] }[] = [
  { title: "About you", short: "Personal details", description: "The basics we use to identify your account and personalize reminders.", fields: [
    { key: "preferredName", label: "Preferred name", placeholder: "What should we call you?", required: true }, { key: "dateOfBirth", label: "Date of birth", type: "date", required: true }, { key: "address", label: "Street address", placeholder: "123 Main Street", required: true }, { key: "city", label: "City", required: true }, { key: "state", label: "State", type: "select", options: ["Texas", "Oklahoma", "Florida", "California", "New York", "Other"], required: true }, { key: "postalCode", label: "ZIP code", required: true }, { key: "preferredContact", label: "Preferred contact method", type: "select", options: ["Text message", "Phone call", "Email"], required: true }, { key: "bestContactTime", label: "Best time to reach you", type: "select", options: ["Morning", "Afternoon", "Evening", "Any time"] },
  ]},
  { title: "Household", short: "Family & dependents", description: "Who depends on you and who should be included in protection planning.", fields: [
    { key: "maritalStatus", label: "Marital status", type: "select", options: ["Single", "Married", "Partnered", "Divorced", "Widowed"], required: true }, { key: "spouseName", label: "Spouse or partner name", placeholder: "If applicable" }, { key: "dependentsCount", label: "Number of dependents", type: "number", placeholder: "0", required: true }, { key: "dependentDetails", label: "Dependent names and ages", type: "textarea", placeholder: "Example: Ava, 6; Noah, 3", helper: "Names are optional during initial setup; ages help estimate the protection timeline." }, { key: "householdNotes", label: "Other household responsibilities", type: "textarea", placeholder: "Aging parents, special needs planning, shared caregiving..." },
  ]},
  { title: "Financial picture", short: "Income & obligations", description: "High-level numbers that help measure whether current protection matches real obligations.", fields: [
    { key: "employmentStatus", label: "Employment status", type: "select", options: ["Employed", "Self-employed", "Business owner", "Retired", "Student", "Not currently employed"], required: true }, { key: "occupation", label: "Occupation or business", required: true }, { key: "annualIncomeRange", label: "Annual personal income", type: "select", options: ["Under $25,000", "$25,000–$49,999", "$50,000–$74,999", "$75,000–$99,999", "$100,000–$149,999", "$150,000+"], required: true }, { key: "householdIncomeRange", label: "Annual household income", type: "select", options: ["Under $50,000", "$50,000–$99,999", "$100,000–$149,999", "$150,000–$249,999", "$250,000+"], required: true }, { key: "mortgageBalance", label: "Mortgage or rent obligation", placeholder: "Balance or monthly rent" }, { key: "otherDebt", label: "Other major debt", placeholder: "Auto, student, business, personal loans" }, { key: "liquidSavings", label: "Savings and liquid assets", type: "select", options: ["Under $10,000", "$10,000–$49,999", "$50,000–$99,999", "$100,000–$249,999", "$250,000+"] }, { key: "monthlyCoverageBudget", label: "Comfortable monthly insurance budget", placeholder: "$0–$0" },
  ]},
  { title: "Protection goals", short: "Needs & priorities", description: "What the coverage should accomplish if life changes unexpectedly.", fields: [
    { key: "primaryGoal", label: "Primary reason for coverage", type: "select", options: ["Protect family income", "Pay off a mortgage", "Final expenses", "Build cash value", "Estate or legacy planning", "Business protection", "Review existing coverage"], required: true }, { key: "incomeReplacementYears", label: "Years of income to replace", type: "number", placeholder: "10" }, { key: "desiredCoverage", label: "Coverage amount you have in mind", placeholder: "$500,000" }, { key: "educationFunding", label: "Education funding goal", placeholder: "Amount or description" }, { key: "finalExpenseGoal", label: "Final-expense goal", placeholder: "$25,000" }, { key: "businessObligations", label: "Business or key-person obligations", type: "textarea", placeholder: "Loans, partners, payroll, buy-sell agreement..." }, { key: "coverageConcerns", label: "What concerns you most?", type: "textarea", placeholder: "Tell us what you want the review to answer", required: true },
  ]},
  { title: "Current coverage", short: "Policies you already own", description: "A starting inventory of personal, employer, and business coverage—even if you bought it elsewhere.", fields: [
    { key: "hasExistingCoverage", label: "Do you currently have life insurance?", type: "select", options: ["Yes", "No", "Not sure"], required: true }, { key: "policyCount", label: "How many policies do you have?", type: "number", placeholder: "0" }, { key: "employerCoverage", label: "Employer-provided coverage", placeholder: "Example: 2× salary" }, { key: "knownCarriers", label: "Known insurance carriers", placeholder: "List any carriers you remember" }, { key: "currentCoverageEstimate", label: "Estimated total death benefit", placeholder: "$0" }, { key: "currentPremiumEstimate", label: "Estimated total monthly premiums", placeholder: "$0" }, { key: "policyConcerns", label: "Concerns with current policies", type: "textarea", placeholder: "Price, performance, beneficiaries, affordability, unclear terms..." }, { key: "uploadAfterOnboarding", label: "I will upload policy documents after onboarding", type: "checkbox", helper: "Document Intelligence can fill in exact policy details later." },
  ]},
  { title: "Beneficiaries", short: "People & contacts", description: "The people who may receive benefits and someone we can contact in an emergency.", fields: [
    { key: "primaryBeneficiary", label: "Primary beneficiary", placeholder: "Full legal name", required: true }, { key: "primaryRelationship", label: "Relationship", required: true }, { key: "primaryPercentage", label: "Primary allocation", placeholder: "100%" }, { key: "contingentBeneficiary", label: "Contingent beneficiary", placeholder: "Full legal name" }, { key: "contingentRelationship", label: "Contingent relationship" }, { key: "trustOrEstate", label: "Trust, estate, or entity beneficiary", placeholder: "If applicable" }, { key: "emergencyContactName", label: "Emergency contact", placeholder: "Full name", required: true }, { key: "emergencyContactPhone", label: "Emergency contact phone", placeholder: "(000) 000-0000", required: true },
  ]},
  { title: "Service preferences", short: "Reviews & consent", description: "How you want your ongoing insurance relationship to work.", fields: [
    { key: "reviewFrequency", label: "Coverage review frequency", type: "select", options: ["Every 6 months", "Annually", "After major life events", "Only when I request one"], required: true }, { key: "communicationStyle", label: "How should we explain information?", type: "select", options: ["Short and direct", "Detailed with examples", "Talk it through with me"], required: true }, { key: "availability", label: "Typical meeting availability", placeholder: "Weekdays after 5 PM" }, { key: "existingAdvisor", label: "Existing agent or advisor", placeholder: "Name and contact, if applicable" }, { key: "lifeEvents", label: "Upcoming life changes", type: "textarea", placeholder: "Marriage, move, home purchase, new child, retirement, business change..." }, { key: "informationConsent", label: "I authorize InsurSuite to organize the information and documents I provide for policy servicing and coverage reviews.", type: "checkbox", required: true }, { key: "accuracyAttestation", label: "I confirm the information provided is accurate to the best of my knowledge.", type: "checkbox", required: true },
  ]},
];
...
export default function HomePage() {
  ...
  if (portalMode === "onboarding" && portalUser && storedProfile) return <OnboardingFlow user={portalUser} profile={storedProfile} onComplete={(profile) => { setStoredProfile(profile); setPortalMode("ready"); }} onSkip={skipSetup} />;
