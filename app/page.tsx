...
function structurePolicyText(text: string): ExtractedPolicy {
  ...
  return { carrier, policyType, policyNumber, insuredName, ownerName: ownerName || insuredName, deathBenefit, monthlyPremium, effectiveDate, premiumDueDate, beneficiaries, cashValue, confidence: Math.round(55 + (found / 7) * 43), sourceText: clean.slice(0, 12000) };
}
            ...
            {policyData.slice(0, 6).map((policy) => {
              ...
              return (
                <button key={policy.id} className="policy-row" onClick={() => onPolicy(policy)}>
                  <span className={`policy-icon ${policy.color}`}><CarrierBadge carrier={policy.carrier} isSample={policy.isSample} /></span>
...
type DocumentRow = { id?: string; name: string; policy: string; policyNumber?: string; type: string; date: string; size: string; contentType?: string; previewUrl?: string };

function DocumentVaultView({ onOpen, notify, uploadedDocuments, policyData }: { onOpen: (modal: string) => void; notify: (message: string) => void; uploadedDocuments: PortalDocument[]; policyData: Policy[] }) {
  const [query, setQuery] = useState("");
  const [previewDocument, setPreviewDocument] = useState<{ id?: string; name: string; contentType?: string; previewUrl?: string } | null>(null);
  const persistentDocuments: DocumentRow[] = uploadedDocuments.map((doc) => ({ id: doc.id, name: doc.fileName, policy: doc.policyNumber ? `Policy #${doc.policyNumber}` : "Unassigned policy", policyNumber: doc.policyNumber, type: doc.contentType === "text/plain" ? "Text policy file" : doc.contentType.startsWith("image/") ? "Policy image" : "AI-scanned policy", date: new Date(doc.createdAt).toLocaleDateString("en-US", { month: "short", day: "numeric", year: "numeric" }), size: doc.fileSize > 1048576 ? `${(doc.fileSize / 1048576).toFixed(1)} MB` : `${Math.max(1, Math.round(doc.fileSize / 1024))} KB`, contentType: doc.contentType, previewUrl: `/api/documents/${doc.id}` }));
...
export default function HomePage() {
  ...
  const saveScannedPolicy = async () => {
    ...
    const newPolicy: Policy = { id, type: extractedPolicy.policyType || "Imported Life Insurance Policy", carrier: extractedPolicy.carrier || "Carrier needs review", benefit: extractedPolicy.deathBenefit || "$0", premium: extractedPolicy.monthlyPremium ? `${extractedPolicy.monthlyPremium}/mo` : "$0/mo", cashValue: extractedPolicy.cashValue || "$0", beneficiaries: extractedPolicy.beneficiaries || "", premiumDueDate: extractedPolicy.premiumDueDate, color: "blue", icon: ShieldCheck };
    try {
